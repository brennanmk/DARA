    ...
    def getData(self, table):
        ...
        if table == 'robots':
            for robot in robots.select():
                data.append({'id': robot.id, 'name': robot.name, 'ip_addr': robot.ip_addr, 'port': robot.port, 'image': robot.image, 'image_target_width': robot.image_target_width})
        elif table == 'augments':
            for augment in augments.select():
                data.append({'id': augment.id, 'robot_id': augment.robot_id, 'name': augment.name, 'location': augment.location, 'config': augment.config})
        else:
            print('Invalid table name')
            return None
